...
export async function generateVerificationPdf(d: VerificationPdfData): Promise<Uint8Array> {
  ...
  const contactBadge = (cx: number, cy: number, type: string) => {
    ...
    const scale = 0.4; // 24 * 0.4 ≈ 9.6px glyph
    const sz = 24 * scale;
  ...
  const drawHeader = () => {
    if (logoImg) {
      // Big logo, compact contact block — like the letterhead sample.
      const boxTop = H - 12, boxH = 110, boxW = 380;
    ...
    const size = 8;
    const lh = 15;
    const startCy = H - 64 + ((items.length - 1) * lh) / 2;
    items.forEach((it, i) => {
    ...
    page.drawLine({ start: { x: margin, y: H - 128 }, end: { x: right, y: H - 128 }, thickness: 2.6, color: BLUE });
  };

  const newPage = () => { page = pdf.addPage([W, H]); drawHeader(); y = H - 144; };
